...
object TemperatureControl : BaseCluster() {
  const val ID = 86L
  ...
  sealed class Command(val id: Long, val name: String) {
    object SetTemperature : Command(0L, "SetTemperature")
  ...
  sealed class SetTemperatureCommandField(val id: Int, val name: String) {
    object TargetTemperature : SetTemperatureCommandField(0, "TargetTemperature")

    object TargetTemperatureLevel : SetTemperatureCommandField(1, "TargetTemperatureLevel")

    companion object {
      fun values(): List<SetTemperatureCommandField> {
        return SetTemperatureCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): SetTemperatureCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
