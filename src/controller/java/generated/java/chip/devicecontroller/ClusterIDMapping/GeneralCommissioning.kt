...
object GeneralCommissioning : BaseCluster() {
  const val ID = 48L
  ...
  sealed class Command(val id: Long, val name: String) {
    object ArmFailSafe : Command(0L, "ArmFailSafe")

    object SetRegulatoryConfig : Command(2L, "SetRegulatoryConfig")

    object CommissioningComplete : Command(4L, "CommissioningComplete")
  ...
  sealed class ArmFailSafeCommandField(val id: Int, val name: String) {
    object ExpiryLengthSeconds : ArmFailSafeCommandField(0, "ExpiryLengthSeconds")

    object Breadcrumb : ArmFailSafeCommandField(1, "Breadcrumb")

    companion object {
      fun values(): List<ArmFailSafeCommandField> {
        return ArmFailSafeCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): ArmFailSafeCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class SetRegulatoryConfigCommandField(val id: Int, val name: String) {
    object NewRegulatoryConfig : SetRegulatoryConfigCommandField(0, "NewRegulatoryConfig")

    object CountryCode : SetRegulatoryConfigCommandField(1, "CountryCode")

    object Breadcrumb : SetRegulatoryConfigCommandField(2, "Breadcrumb")

    companion object {
      fun values(): List<SetRegulatoryConfigCommandField> {
        return SetRegulatoryConfigCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): SetRegulatoryConfigCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
