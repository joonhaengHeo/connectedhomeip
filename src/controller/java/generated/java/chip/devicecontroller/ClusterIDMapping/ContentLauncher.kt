...
object ContentLauncher : BaseCluster() {
  const val ID = 1290L
  ...
  sealed class Command(val id: Long, val name: String) {
    object LaunchContent : Command(0L, "LaunchContent")

    object LaunchURL : Command(1L, "LaunchURL")
  ...
  sealed class LaunchContentCommandField(val id: Int, val name: String) {
    object Search : LaunchContentCommandField(0, "Search")

    object AutoPlay : LaunchContentCommandField(1, "AutoPlay")

    object Data : LaunchContentCommandField(2, "Data")

    companion object {
      fun values(): List<LaunchContentCommandField> {
        return LaunchContentCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): LaunchContentCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class LaunchURLCommandField(val id: Int, val name: String) {
    object ContentURL : LaunchURLCommandField(0, "ContentURL")

    object DisplayString : LaunchURLCommandField(1, "DisplayString")

    object BrandingInformation : LaunchURLCommandField(2, "BrandingInformation")

    companion object {
      fun values(): List<LaunchURLCommandField> {
        return LaunchURLCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): LaunchURLCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
