...
object DiagnosticLogs : BaseCluster() {
  const val ID = 50L
  ...
  sealed class Command(val id: Long, val name: String) {
    object RetrieveLogsRequest : Command(0L, "RetrieveLogsRequest")
  ...
  sealed class RetrieveLogsRequestCommandField(val id: Int, val name: String) {
    object Intent : RetrieveLogsRequestCommandField(0, "Intent")

    object RequestedProtocol : RetrieveLogsRequestCommandField(1, "RequestedProtocol")

    object TransferFileDesignator : RetrieveLogsRequestCommandField(2, "TransferFileDesignator")

    companion object {
      fun values(): List<RetrieveLogsRequestCommandField> {
        return RetrieveLogsRequestCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): RetrieveLogsRequestCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
