...
object Identify : BaseCluster() {
  ...
  sealed class Command(val id: Long, val name: String) {
    object Identify : Command(0L, "Identify")

    object TriggerEffect : Command(64L, "TriggerEffect")
  ...
  sealed class IdentifyCommandField(val id: Int, val name: String) {
    object IdentifyTime : IdentifyCommandField(0, "IdentifyTime")

    companion object {
      fun values(): List<IdentifyCommandField> {
        return IdentifyCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): IdentifyCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class TriggerEffectCommandField(val id: Int, val name: String) {
    object EffectIdentifier : TriggerEffectCommandField(0, "EffectIdentifier")

    object EffectVariant : TriggerEffectCommandField(1, "EffectVariant")

    companion object {
      fun values(): List<TriggerEffectCommandField> {
        return TriggerEffectCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): TriggerEffectCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
