...
object DishwasherAlarm : BaseCluster() {
  const val ID = 93L
  ...
  sealed class Command(val id: Long, val name: String) {
    object Reset : Command(0L, "Reset")

    object ModifyEnabledAlarms : Command(1L, "ModifyEnabledAlarms")
  ...
  sealed class ResetCommandField(val id: Int, val name: String) {
    object Alarms : ResetCommandField(0, "Alarms")

    companion object {
      fun values(): List<ResetCommandField> {
        return ResetCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): ResetCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class ModifyEnabledAlarmsCommandField(val id: Int, val name: String) {
    object Mask : ModifyEnabledAlarmsCommandField(0, "Mask")

    companion object {
      fun values(): List<ModifyEnabledAlarmsCommandField> {
        return ModifyEnabledAlarmsCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): ModifyEnabledAlarmsCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
