...
object AccountLogin : BaseCluster() {
  const val ID = 1294L
  ...
  sealed class Command(val id: Long, val name: String) {
    object GetSetupPIN : Command(0L, "GetSetupPIN")

    object Login : Command(2L, "Login")

    object Logout : Command(3L, "Logout")
  ...
  sealed class GetSetupPINCommandField(val id: Int, val name: String) {
    object TempAccountIdentifier : GetSetupPINCommandField(0, "TempAccountIdentifier")

    companion object {
      fun values(): List<GetSetupPINCommandField> {
        return GetSetupPINCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): GetSetupPINCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class LoginCommandField(val id: Int, val name: String) {
    object TempAccountIdentifier : LoginCommandField(0, "TempAccountIdentifier")

    object SetupPIN : LoginCommandField(1, "SetupPIN")

    companion object {
      fun values(): List<LoginCommandField> {
        return LoginCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): LoginCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
