...
object MediaInput : BaseCluster() {
  ...
  sealed class Command(val id: Long, val name: String) {
    object SelectInput : Command(0L, "SelectInput")

    object ShowInputStatus : Command(1L, "ShowInputStatus")

    object HideInputStatus : Command(2L, "HideInputStatus")

    object RenameInput : Command(3L, "RenameInput")
  ...
  sealed class SelectInputCommandField(val id: Int, val name: String) {
    object Index : SelectInputCommandField(0, "Index")

    companion object {
      fun values(): List<SelectInputCommandField> {
        return SelectInputCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): SelectInputCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
  ...
  sealed class RenameInputCommandField(val id: Int, val name: String) {
    object Index : RenameInputCommandField(0, "Index")

    object Name : RenameInputCommandField(1, "Name")

    companion object {
      fun values(): List<RenameInputCommandField> {
        return RenameInputCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): RenameInputCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
