...
object GeneralDiagnostics : BaseCluster() {
  const val ID = 51L
  ...
  sealed class Event(val id: Long, val name: String) {
    object HardwareFaultChange : Event(0L, "HardwareFaultChange")

    object RadioFaultChange : Event(1L, "RadioFaultChange")

    object NetworkFaultChange : Event(2L, "NetworkFaultChange")

    object BootReason : Event(3L, "BootReason")
  ...
  sealed class Command(val id: Long, val name: String) {
    object TestEventTrigger : Command(0L, "TestEventTrigger")
  ...
  sealed class TestEventTriggerCommandField(val id: Int, val name: String) {
    object EnableKey : TestEventTriggerCommandField(0, "EnableKey")

    object EventTrigger : TestEventTriggerCommandField(1, "EventTrigger")

    companion object {
      fun values(): List<TestEventTriggerCommandField> {
        return TestEventTriggerCommandField::class.sealedSubclasses.map { it.objectInstance!! }
      }
      ...
      @Throws(IllegalArgumentException::class)
      fun valueOf(value: String): TestEventTriggerCommandField {
        for (field in values()) {
          if (field.name == value) {
            return field
          }
